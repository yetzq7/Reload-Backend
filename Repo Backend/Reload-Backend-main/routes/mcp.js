...
app.post("/fortnite/api/game/v2/profile/*/client/EquipBattleRoyaleCustomization", verifyToken, async (req, res) => {
    ...
    if (profile.items[req.body.itemToSlot]) {
        ...
        if (Array.isArray(Variants)) {
            if (!profile.items[req.body.itemToSlot].attributes.variants) profile.items[req.body.itemToSlot].attributes.variants = [];
            ...
            if (profile.items[activeLoadoutId] && profile.items[activeLoadoutId].attributes.locker_slots_data.slots[req.body.slotName]) {
                profile.items[activeLoadoutId].attributes.locker_slots_data.slots[req.body.slotName].activeVariants = [{
                    variants: Variants.map(v => ({ channel: v.channel, active: v.active }))
                }];
...
app.post("/fortnite/api/game/v2/profile/*/client/SetCosmeticLockerSlot", verifyToken, async (req, res) => {
    ...
    if (profile.items[itemToSlotID]) {
        ...
        if (Array.isArray(Variants)) {
            if (!profile.items[itemToSlotID].attributes.variants) profile.items[itemToSlotID].attributes.variants = [];
            ...
            for (let i in Variants) {
                ...
                if (index == -1) {
                    ...
                    if (!profile.items[itemToSlotID].attributes.variants[index].owned.includes(Variants[i].active)) {
                        profile.items[itemToSlotID].attributes.variants[index].owned.push(Variants[i].active);
                    }
                }
            }

            if (profile.items[req.body.lockerItem] && profile.items[req.body.lockerItem].attributes.locker_slots_data.slots[req.body.category]) {
                profile.items[req.body.lockerItem].attributes.locker_slots_data.slots[req.body.category].activeVariants = [{
                    variants: Variants.map(v => ({ channel: v.channel, active: v.active }))
                }];
            }
